...
static_assert(kTransmissionMaxLength <= 255, "maximum exceeded");
...
// I2C_OVER_UART_ALLOC_MIN_SIZE to kTransmissionBufferMaxSize ensures that messages
...
#if __AVR__ && !defined(ESP8266) && !defined(ESP32)
...
static constexpr uint8_t kMaxTransmissionLength = kSerialTwoWireTransmitCommandLength > kSerialTwoWireTransmitCommandLength ? kSerialTwoWireTransmitCommandLength : kSerialTwoWireTransmitCommandLength;
static_assert(kMaxTransmissionLength >= 4, "min. size required");
